...
transaction(recipientAddr: Address, momentIDs: [UInt64]) {
    ...
    prepare(acct: AuthAccount) {
        ...
        let receiverRef = recipient.getCapability(Inspirax.CollectionPublicPath)
        ...
        if let collection = acct.borrow<&InspiraxShardedCollection.ShardedCollection>(from: InspiraxShardedCollection.ShardedCollectionStoragePath) {

            receiverRef.batchDeposit(tokens: <-collection.batchWithdraw(ids: momentIDs))

        } else {

            let collection = acct.borrow<&Inspirax.Collection>(from: Inspirax.CollectionStoragePath)!
