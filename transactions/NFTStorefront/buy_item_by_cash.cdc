...
transaction(listingResourceID: UInt64, storefrontAddress: Address, buyerAddress: Address) {
    ...
    let inspiraxNFTCollection: &{Inspirax.MomentCollectionPublic}
    ...
    prepare(admin: AuthAccount) {
        ...
        self.inspiraxNFTCollection = self.buyerAccount.getCapability(Inspirax.CollectionPublicPath).borrow<&{Inspirax.MomentCollectionPublic}>()
    ...
    execute {
        ...
        let paymentVault <- minter.mintTokens(amount: self.price) as! @InspiraxUtilityCoin.Vault
        destroy minter
