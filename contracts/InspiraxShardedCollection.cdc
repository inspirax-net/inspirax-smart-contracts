/**
    Description: Central Collection for a large number of Inspirax NFTs
...
pub contract InspiraxShardedCollection {

    /// Named Paths
    pub let ShardedCollectionStoragePath: StoragePath
    pub let ShardedCollectionPublicPath: PublicPath

    /// ShardedCollection stores a dictionary of Inspirax Collections
    /// A Moment is stored in the field that corresponds to its id % numBuckets
    pub resource ShardedCollection: Inspirax.MomentCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic { 

        /// Dictionary of Inspirax collections
        pub var collections: @{UInt64: Inspirax.Collection}
        ...
        init(numBuckets: UInt64) {
            ...
            while i < numBuckets {
                ...
                self.collections[i] <-! Inspirax.createEmptyCollection() as! @Inspirax.Collection
        ...
        pub fun batchWithdraw(ids: [UInt64]): @NonFungibleToken.Collection {
            var batchCollection <- Inspirax.createEmptyCollection()
    ...
    init() {
        // Set named paths
        self.ShardedCollectionStoragePath = /storage/InspiraxShardedMomentCollection
        self.ShardedCollectionPublicPath = /public/InspiraxShardedMomentCollection
    }
}